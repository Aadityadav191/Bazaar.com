...
const Shop = () => {
  ...
  if (error) return (
    <div className="flex flex-col justify-center items-center min-h-screen bg-zinc-50 text-center px-4">
      <p className="text-zinc-900 font-bold mb-1 text-sm">Catalog Initialization Failure</p>
  ...
  const currentPagedProducts = filteredProducts.slice(indexOfFirstItem, indexOfLastItem);
  ...
  return (
    <div className="bg-zinc-50 min-h-screen text-zinc-900 font-sans selection:bg-zinc-200">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mb-1.5 flex items-center gap-1.5">
              Bazaar Studio <ChevronRight className="w-3 h-3 text-zinc-300" /> Catalog Index
            </div>
            <h1 className="text-3xl font-black tracking-tight text-white sm:text-4xl">Explore Collection Frame</h1>
          ...
          <div className="text-left md:text-right">
            <span className="text-2xl font-black tracking-tight text-white font-mono">{totalItems}</span>
            <p className="text-[10px] uppercase font-bold tracking-wider text-zinc-200">Available units live</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="hidden lg:block w-60 flex-shrink-0 space-y-7 top-24 sticky h-fit">
            ...
            <div className="space-y-3">
              <h3 className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Hub Frames</h3>
            ...
            <div className="space-y-3">
              <h3 className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Price Threshold</h3>
              <div className="flex flex-col gap-0.5">
                {[
          ...
          <main className="flex-1 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white border border-zinc-200/80 p-3.5 rounded-2xl shadow-xs">
              ...
              <div className="flex gap-2.5 w-full sm:w-auto justify-end">
                ...
                <div className="relative flex items-center">
                  <ArrowUpDown size={12} className="absolute left-3.5 pointer-events-none text-zinc-400" />
                  <select 
                    value={sortBy}
                  ...
                  <div className="absolute right-3 pointer-events-none text-zinc-400 text-[8px]">▼</div>
                </div>
              </div>
            </div>
            ...
            <div className="text-zinc-400 text-[10px] font-bold uppercase tracking-wider flex items-center justify-between px-1">
              <span>Showing {indexOfFirstItem + 1}–{Math.min(indexOfLastItem, totalItems)} of {totalItems} items indexed</span>
            ...
            {currentPagedProducts.length === 0 ? (
              <div className="text-center bg-white border border-dashed border-zinc-200/80 py-24 rounded-2xl text-zinc-400 font-medium text-xs shadow-inner">
                ...
                Try adjusting your search query or filter settings to discover available items.
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="relative aspect-[4/5] overflow-hidden bg-zinc-50 border-b border-zinc-100">
                        ...
                        >
                          <Heart className={`w-3.5 h-3.5 transition-colors ${isFav ? "fill-rose-500 text-rose-500" : "text-zinc-500"}`} />
                        </button>
                      ...
                      <div className="p-4 flex-grow flex flex-col justify-between space-y-2">
                        <div className="space-y-1">
                          <div className="flex items-center gap-0.5 text-amber-500 text-[10px]">
                            ...
                            <span className="text-zinc-400 font-bold ml-1">(4.5)</span>
                          </div>
                        ...
                        <div className="pt-2 border-t border-zinc-100 flex items-center justify-between">
                          <span className="text-sm font-black text-zinc-950 font-mono">Rs {product.price}</span>
      ...
      <div className={`fixed inset-0 bg-zinc-950/40 backdrop-blur-xs z-50 transition-opacity duration-300 lg:hidden ${mobileFiltersOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
        <div className={`fixed right-0 top-0 bottom-0 w-76 bg-white shadow-xl p-6 space-y-6 transform transition-transform duration-300 ease-out flex flex-col justify-between ${mobileFiltersOpen ? "translate-x-0" : "translate-x-full"}`}>
          <div className="space-y-6">
            <div className="flex items-center justify-between pb-3 border-b border-zinc-100">
              <h2 className="text-xs font-bold uppercase tracking-widest flex items-center gap-2"><Grid size={14}/> Filter Options</h2>
              <button onClick={() => setMobileFiltersOpen(false)} className="p-1 text-zinc-400 hover:text-zinc-900"><X size={18}/></button>
            </div>
